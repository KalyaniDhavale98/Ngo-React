...
class UpdateDonationBox extends React.Component {
  ...
  componentDidMount() {
    axios
      .get(
        `http://localhost:8080/donationBox/${this.props.match.params.registrationNumber}`
      )
      .then((res) => {
  ...
  handleSubmit = (event) => {
    ...
    axios
      .put(
        `http://localhost:8080/donationBox/update/${this.props.match.params.registrationNumber}`,
        this.state.boxes
      )
      .then((res) => {
        console.log(res.data);
